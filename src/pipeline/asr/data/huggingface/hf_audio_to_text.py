...
from pipeline.asr.data.audio_to_text import _speech_collate_fn
from pipeline.asr.preprocessing.perturb import AudioAugmentor
...
from pipeline.common.preprocessing import parsers
from pipeline.core_classes.dataset import Dataset, IterableDataset
from pipeline.neural_types import AudioSignal, LabelsType, LengthsType, NeuralType
...
class _HFAudioTextDataset(Dataset):
    ...
    def __init__(
        ...
        for data_cfg in data_config_list:
            ...
            print(f"Loading HuggingFace Dataset with cfg: {data_cfg}")
            dataset_list.append(hf_datasets.load_dataset(**data_cfg))
            print(f"Dataset loaded with {len(dataset_list[-1])} samples")
        self.dataset = concatenate_datasets(dataset_list)

        print(f"Total number of samples loaded: {len(self.dataset)}")
...
class _HFIterableAudioTextDataset(IterableDataset):
    ...
    def __init__(
        ...
        for data_cfg in data_config_list:
            ...
            print(f"Streaming HuggingFace IterableDataset with cfg: {data_cfg}")
            dataset_list.append(hf_datasets.load_dataset(**data_cfg))

        self.dataset = concatenate_datasets(dataset_list)
        print(f"Total number of samples cannot be extracted from HF streaming dataset")
