...
from pipeline.asr.preprocessing.segment import available_formats as valid_sf_formats
from pipeline.asr.utils.audio_utils import ChannelSelectorType
from pipeline.common import tokenizers
from pipeline.common.preprocessing import collections, parsers
from pipeline.core_classes.dataset import Dataset, IterableDataset
from pipeline.neural_types import *
from pipeline.utils.data_utils import (
    DataStoreObject,
...
from pipeline.utils.distributed import webdataset_split_by_workers
from pipeline.utils.get_rank import is_global_rank_zero
...
def expand_sharded_filepaths(sharded_filepaths, shard_strategy: str, world_size: int, global_rank: int):
    ...
    if world_size > 1:
        if shard_strategy == 'scatter':
            print("All tarred dataset shards will be scattered evenly across all nodes.")
            ...
            print("All tarred dataset shards will be replicated across all nodes.")
        else:
        ...
        def cache_data(manifest_filepaths, cache_audio, num_workers, max_num_workers):
            ...
            for manifest_file in manifest_filepaths:
                ...
                if is_datastore_path(manifest_file):
                    print('Cache manifest file: %s', manifest_file)
                    cached_manifest_file = DataStoreObject(manifest_file).get()
                    print('Cached at: %s', str(cached_manifest_file))

                    if cache_audio:
                        # Each audio file from manifest will be cached.
                        print('Cache audio from manifest file: %s', manifest_file)
                        ...
                        if num_workers is not None and num_workers > 1:
                            print('Using multiprocessing with num_workers: %d.', num_workers)
                    ...
                    # Nothing to do here
                    print('Manifest is not on a data store: %s', manifest_file)

        if torch.distributed.is_available() and torch.distributed.is_initialized():
            print('Distributed environment is available and initialized.')
            ...
            if shared_cache:
                print('Cache is shared among nodes, cache data on global rank zero.')
                is_rank_zero = is_global_rank_zero()
            else:
                print('Cache is not shared among nodes, cache data on local rank zero.')
                local_rank = int(os.environ.get("LOCAL_RANK", 0))
                is_rank_zero = local_rank == 0

            if is_rank_zero:
                print('Cache data from %s rank 0', 'global' if shared_cache else 'local')
                cache_data(
...
def shard_manifests_if_needed(
    ...
    if shard_manifests:
        if not torch.distributed.is_available():
            print("Not running in torch.distributed mode. Manifest sharding not available")
...
class AudioToBPEDataset(_AudioTextDataset):
    ...
    def __init__(
        self,
        manifest_filepath: str,
        tokenizer: 'pipeline.common.tokenizers.TokenizerSpec',
        sample_rate: int,
        int_values: bool = False,
        augmentor: 'pipeline.asr.perturb.AudioAugmentor' = None,
        max_duration: Optional[int] = None,
        min_duration: Optional[int] = None,
        max_utts: int = 0,
        trim: bool = False,
...
class _TarredAudioToTextDataset(IterableDataset):
    ...
    def _compute_len(self):
        if self.shard_manifests and torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            my_len = my_len.int()
            print(f'Sharded manifests: Total length: {my_len}')
        else:
