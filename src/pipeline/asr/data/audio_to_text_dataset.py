...
from pipeline.asr.data import audio_to_text, audio_to_text_dali
from pipeline.asr.data.huggingface.hf_audio_to_text_dataset import (
...
from pipeline.common.data.dataset import CodeSwitchedDataset, ConcatDataset
...
def get_concat_char_dataset(
    ...
    if 'labels' not in config:
        print(f"dataset does not have explicitly defined labels")
    ...
    if len(manifest_filepaths) == 1 and not isinstance(manifest_filepaths[0], str):
        print(f"removing an extra nesting level from {manifest_filepaths}")
...
def get_audio_to_text_char_dataset_from_config(
    ...
    if is_concat:
        ...
        if config['concat_sampling_technique'] == 'random':
            if not 'concat_sampling_probabilities' in config:
                print(f"Concat dataset requires `concat_sampling_probabilities` list. Config: {config}")
                return None
            else:
                if not isclose(sum(config['concat_sampling_probabilities']), 1, abs_tol=1e-6):
                    print(f"`concat_sampling_probabilities` need to sum to 1. Config: {config}")
                    return None
    ...
    if config.get('is_code_switched', False):
        ...
        if (
            ...
            print(f"`.code_switched.probs` need to sum to 1. Config: {config['code_switched']}")
            return None
