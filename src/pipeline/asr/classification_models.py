...
from pipeline.asr.data import audio_to_label_dataset, feature_to_label_dataset
from pipeline.asr.models.asr_model import ASRModel, ExportableEncDecModel
from pipeline.asr.mixins import TranscriptionMixin, TranscriptionReturnType
from pipeline.asr.mixins.transcription import InternalTranscribeConfig
from pipeline.asr.preprocessing.features import WaveformFeaturizer
from pipeline.asr.preprocessing.perturb import process_augmentations
from pipeline.common.losses import CrossEntropyLoss, MSELoss
from pipeline.common.metrics import TopKClassificationAccuracy
...
from pipeline.utils.cast_utils import cast_all
    ...
    def _setup_dataloader_from_config(self, config: DictConfig):
        ...
        if config.get('is_tarred', False):
            ...
            if 'vad_stream' in config and config['vad_stream']:
                print("VAD inference does not support tarred dataset now")
                return None
            ...
            if 'manifest_filepath' in config and config['manifest_filepath'] is None:
                print(f"Could not load dataset as `manifest_filepath` is None. Provided config : {config}")
                return None
...
class EncDecClassificationModel(_EncDecBaseModel):
    ...
    def change_labels(self, new_labels: List[str]):
        ...
        use it if you want to use pre-trained encoder when fine-tuning on a data in another dataset.
        ...
        if self._cfg.labels == new_labels:
            ...
            print(f"Changed decoder output to {self.decoder.num_classes} labels.")
...
class EncDecFrameClassificationModel(EncDecClassificationModel):
    ...
    def _setup_loss(self):
        if "loss" in self.cfg:
            ...
            print(f"Using cross-entropy with weights: {weight}")
        else:
            weight = [1.0] * self.num_classes
