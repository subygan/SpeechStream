...
import pipeline.asr.models as asr_models
from pipeline.asr.mixins.asr_adapter_mixins import ASRAdapterModelMixin
from pipeline.asr.mixins.streaming import StreamingEncoder
from pipeline.asr.utils import asr_module_utils
from pipeline.asr.utils.rnnt_utils import Hypothesis
from pipeline.common import tokenizers
from pipeline.utils import app_state
...
class ASRBPEMixin(ABC):
    ...
    def _setup_aggregate_tokenizer(self, tokenizer_cfg: DictConfig):
        ...
        print('_setup_tokenizer: detected an aggregate tokenizer')
    ...
    def _extract_tokenizer_from_config(self, tokenizer_cfg: DictConfig, dir: str):
        ...
        if len(nemo_file_objects) > 0:
            print(f"Copying the following nemo file objects to {dir}: {nemo_file_objects}")
            ...
            for nemo_object_name in nemo_file_objects:
                ...
                for member in members:
                    ...
                    print(f"Saved {nemo_object_name} at {os.path.join(dir, new_name)}")
...
class ASRModuleMixin(ASRAdapterModelMixin):
    ...
    def change_attention_model(
        ...
        if not hasattr(self.encoder, "change_attention_model"):
            print("Model encoder doesn't have a change_attention_model method ")
            return
    ...
    def change_subsampling_conv_chunking_factor(
        ...
        if not hasattr(self.encoder, "change_subsampling_conv_chunking_factor"):
            print("Model encoder doesn't have a change_subsampling_conv_chunking_factor method ")
            return
    ...
    def conformer_stream_step(
        ...
        if not isinstance(self, asr_models.EncDecCTCModel) and return_log_probs is True:
            print("return_log_probs can only be True for CTC models.")
    ...
    def _setup_streaming_transcribe_dataloader(
        ...
        for sample_idx, sample in enumerate(paths2audio_files):
            ...
            print(f'Added this sample to the buffer: {sample}')
            if (sample_idx + 1) % batch_size == 0 or sample_idx == len(paths2audio_files) - 1:
                print(f"Starting to stream samples {sample_idx - len(streaming_buffer) + 1} to {sample_idx}...")
