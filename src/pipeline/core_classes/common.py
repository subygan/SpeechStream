...
from pipeline.core_classes.hf_io_mixin import HuggingFaceFileIO
from pipeline.connectors.save_restore_connector import SaveRestoreConnector
from pipeline.neural_types import NeuralType, NeuralTypeComparisonResult
from pipeline.utils.cloud import maybe_download_from_cloud
from pipeline.utils.data_utils import resolve_cache_dir
from pipeline.utils.model_utils import import_class_by_path, maybe_update_config_version
...
class Model(Typing, Serialization, FileIO, HuggingFaceFileIO):
    ...
    def _get_ngc_pretrained_model_info(cls, model_name: str, refresh_cache: bool = False) -> Tuple[type, str]:
        ...
        print("Instantiating model from pre-trained checkpoint")
    ...
    def _get_hf_hub_pretrained_model_info(cls, model_name: str, refresh_cache: bool = False) -> Tuple[type, str]:
        ...
        if nemo_file_exists:
            ...
            if not os.path.exists(save_path):
                print(f"Downloading {model_name} from HuggingFace Hub to path: {save_path}")
