...
import pipeline.core_classes.model as required_model   # to avoid circular import do not import ModelPT directly
...
from pipeline.utils.model_utils import inject_model_parallel_rank
...
class SaveRestoreConnector:
    ...
    def save_to(self, model: "required_model.ModelPT", save_path: str):
        """
    ...
    def restore_from(
        ...
        print(f'Model {instance.__class__.__name__} was successfully restored from {restore_path}.')
        return instance
    ...
    def extract_state_dict_from(self, restore_path: str, save_dir: str, split_by_module: bool = False):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                print(f'Checkpoints from {restore_path} were successfully extracted into {save_dir}.')
            finally:
                os.chdir(cwd)
    ...
    def _handle_artifacts(self, model, nemo_file_folder):
        ...
        for module in model.modules():
            if isinstance(module, "required_model.ModelPT"):  # if NeMo model
        ...
        if len(tarfile_artifacts) > 0 and len(restoration_paths) == 0:
            ...
            print("Model contains registered artifacts, but no restoration paths found")
        if len(tarfile_artifacts) > 0 and len(restoration_paths) > 0:
    ...
    @staticmethod
    def _update_subconfigs(model: "required_model.ModelPT", path2yaml_file):
        """
