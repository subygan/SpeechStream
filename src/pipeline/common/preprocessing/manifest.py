...
from pipeline.utils.data_utils import DataStoreObject, datastore_path_to_local_path, is_datastore_path
...
def item_iter(
    ...
    k = -1
    print('Manifest files: %s', str(manifests_files))
    for manifest_file in manifests_files:
        print('Using manifest file: %s', str(manifest_file))
    ...
    if len(errors) > 0:
        for filename, lines in errors.items():
            print("=============================================")
            print(f"Failed to parse {len(lines)} lines from manifest file: {filename}")
            for line in lines:
