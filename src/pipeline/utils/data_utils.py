...
from pipeline.utils import constants
...
def ais_binary() -> str:
    ...
    print('AIS binary not found with `which ais`.')
...
def get_datastore_object(path: str, force: bool = False, num_retries: int = 5) -> str:
    ...
    if path.startswith('ais://'):
        ...
        if not os.path.isfile(local_path) or force:
            ...
            print('Downloading from AIS')
            print('\tendpoint    %s', endpoint)
            print('\tpath:       %s', path)
            print('\tlocal path: %s', local_path)
            print('\tcmd:        %s', subprocess.list2cmdline(cmd))
            ...
            for n in range(num_retries):
                if not done:
                    try:
                        ...
                        print('Attempt %d of %d failed with: %s', n + 1, num_retries, str(err))
