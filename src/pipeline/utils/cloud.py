...
def maybe_download_from_cloud(url, filename, subfolder=None, cache_dir=None, refresh_cache=False) -> str:
    ...
    if os.path.exists(destination_file):
        print(f"Found existing object {destination_file}.")
        if refresh_cache:
            print("Asked to refresh the cache.")
            print(f"Deleting file: {destination_file}")
            os.remove(destination_file)
        else:
            print(f"Re-using file from: {destination_file}")
            return str(destination_file)
    # download file
    wget_uri = url + filename
    print(f"Downloading from: {wget_uri} to {str(destination_file)}")
