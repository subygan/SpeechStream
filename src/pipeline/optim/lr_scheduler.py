...
from pipeline.config import SchedulerParams, get_scheduler_config, register_scheduler_params
...
def prepare_lr_scheduler(
    ...
    if scheduler_config is not None:
        ...
        print('Scheduler not initialized as no `sched` config supplied to setup_optimizer()')
        return None
    ...
    if max_steps_from_cfg is not None:
        if max_steps_from_cfg == -1:
            print('`max_steps` is set to -1 in the scheduler config, scheduler will not be instantiated')
            return None
