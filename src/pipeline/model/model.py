...
class EncDecDiarLabelModel(ModelPT):
    """
    ...
    def get_available_model_names(self):
        ...
        return self.available_model.pretrained_model_name
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        """
        self.available_model = PretrainedModelInfo(
        ...
        # Call `self.save_hyperparameters` in core.py again since cfg should contain speaker model's config.
    ...
    def _init_speaker_model(self):
        ...
        if model_path is not None and model_path.endswith('.nemo'):
            ...
            print("Speaker Model restored locally from {}".format(model_path))
        elif model_path.endswith('.ckpt'):
            self._speaker_model = EncDecSpeakerLabelModel.load_from_checkpoint(model_path, map_location=rank_id)
            print("Speaker Model restored locally from {}".format(model_path))
        else:
            if model_path != EncDecSpeakerLabelModel.get_available_model_names():
                print(
                    "requested {} model name not available in pretrained models, instead".format(model_path)
                )
                model_path = "titanet_large"
            print("Loading pretrained {} model from NGC".format(model_path))
    ...
    def __setup_dataloader_from_config(self, config):
        ...
        if 'manifest_filepath' in config and config['manifest_filepath'] is None:
            print(f"Could not load dataset as `manifest_filepath` was None. Provided config : {config}")
            return None
...
class ClusterEmbedding(torch.nn.Module):
    ...
    def __init__(
            self, cfg_diar_infer: DictConfig, cfg_msdd_model: DictConfig,
            speaker_model: Optional[EncDecSpeakerLabelModel]
    ):
        super().__init__()
        ...
        '''Prepare clustering Diarizer and create the required variables'''
        self.cfg_diar_infer.diarizer.manifest_filepath = self._cfg_msdd.test_ds.manifest_filepath
        self.cfg_diar_infer.diarizer.out_dir = self._cfg_msdd.test_ds.emb_dir
    ...
    def run_clustering_diarizer(self, manifest_filepath: str, emb_dir: str):
        ...
        print(f"Multiscale Weights: {self.clus_diar_model.multiscale_args_dict['multiscale_weights']}")
        print(f"Clustering Parameters: {clustering_params_str}")
    ...
    def get_cluster_avg_embs(
            self, emb_scale_seq_dict: Dict, clus_labels: List, speaker_mapping_dict: Dict,
            session_scale_mapping_dict: Dict
    ):
        """
    ...
    def check_clustering_labels(self, out_dir):
        ...
        if not file_exists:
            print(f"Clustering label file {clus_label_path} does not exist.")
...
class NeuralDiarizer(LightningModule):
    ...
    def _init_msdd_model(self, cfg: Union[DictConfig, NeuralDiarizerInferenceConfig]):
        ...
        if model_path.endswith('.nemo'):
            print(f"Using local nemo file from {model_path}")
            self.msdd_model = EncDecDiarLabelModel.restore_from(restore_path=model_path, map_location=cfg.device)
        elif model_path.endswith('.ckpt'):
            print(f"Using local checkpoint from {model_path}")
            self.msdd_model = EncDecDiarLabelModel.load_from_checkpoint(
                checkpoint_path=model_path, map_location=cfg.device
            )
        else:
            if model_path not in EncDecDiarLabelModel.get_available_model_names():
                print(f"requested {model_path} model name not available in pretrained models, instead")
            print("Loading pretrained {} model from NGC".format(model_path))
    ...
    def get_range_average(
            self, signals: torch.Tensor, emb_vectors: torch.Tensor, diar_window_index: int,
            test_data_collection: List[Any]
    ...
    def run_pairwise_diarization(self) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        ...
        if self._cfg.diarizer.msdd_model.parameters.seq_eval_mode:
            ...
            print(f"Test Inference F1 score. {f1_score:.4f}, simple Acc. {simple_acc:.4f}")
