...
class ModelPT(LightningModule, Model):
    ...
    def setup_optimization(
        ...
        if optim_config is None:
            print('No optimizer config provided, therefore no optimizer was created')
            return
        ...
        if self._trainer is None:
            print(f"Trainer wasn't specified in model constructor. Make sure that you really wanted it.")
        ...
        if optimizer_cls is not None:
            if inspect.isclass(optimizer_cls):
                ...
                print("Optimizer config = %s", str(optimizer))
                ...
                try:
                    ...
                    print("Optimizer config = %s", str(optimizer_instance))
    ...
    def load_part_of_state_dict(self, state_dict, include, exclude, load_from_string=None):
        ...
        if load_from_string is not None:
            print(f'Model checkpoint partially restored from {load_from_string}')
            if len(excluded_param_names) > 0:
                ...
                print(f'Make sure that this is what you wanted!')
        else:
            if len(excluded_param_names) > 0:
                print(
        ...
        if 'init_from_nemo_model' in cfg and cfg.init_from_nemo_model is not None:
            with open_dict(cfg):
                if isinstance(cfg.init_from_nemo_model, str):
                    ...
                    print(f'Model checkpoint restored from nemo file with path : `{model_path}`')
                    del restored_model
        ...
        if 'init_from_pretrained_model' in cfg and cfg.init_from_pretrained_model is not None:
            with open_dict(cfg):
                ...
                if isinstance(cfg.init_from_pretrained_model, str):
                    ...
                    # Restore checkpoint into current model
                    self.load_state_dict(restored_model.state_dict(), strict=False)
                    print(f'Model checkpoint restored from pretrained checkpoint with name : `{model_name}`')
    ...
    def prepare_test(self, trainer: 'Trainer') -> bool:
        ...
        if not hasattr(self._cfg, 'test_ds'):
            print("No `test_ds` config found within the manifest.")
            return False
    ...
    def set_world_size(self, trainer: Trainer):
        ...
        if trainer is not None:
            if isinstance(trainer, Trainer):
                ...
                print(f'World size can only be set by PyTorch Lightning Trainer.')
        app_state = AppState()
        ...
        if self.cfg.get('nsys_profile', None) is not None:
            if self.cfg.nsys_profile.get('enabled', False):
                ...
                if type(self._nsys_profile_start_step) == int:
                    print(f'Nsys profiling setup with start_step: {self._nsys_profile_start_step}')
                else:
                    raise ValueError(
                ...
                if type(self._nsys_profile_end_step) == int:
                    print(f'Nsys profiling setup with end_step: {self._nsys_profile_end_step}')
                else:
