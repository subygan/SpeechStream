...
class ClusteringDiarizer(torch.nn.Module, Model, DiarizationMixin):
    ...
    def _init_vad_model(self):
        ...
        if model_path.endswith('.nemo'):
            ...
            print("VAD model loaded locally from {}".format(model_path))
        else:
    ...
    def _run_vad(self, manifest_file):
        ...
        for key in self.AUDIO_RTTM_MAP:
            if os.path.exists(os.path.join(table_out_dir, key + ".txt")):
                ...
                print(f"no vad file found for {key} due to zero or negative duration")
    ...
    def _perform_speech_activity_detection(self):
        ...
        if self.has_vad_model:
            ...
            if self._auto_split:
                print("Split long audio file to avoid CUDA memory issue")
                print("Try smaller split_duration if you still have CUDA memory issue")
                config = {
    ...
    def _extract_embeddings(self, manifest_file: str, scale_idx: int, num_scales: int):
        ...
        """
        print("Extracting embeddings for Diarization")
    ...
    def diarize(self, paths2audio_files: List[str] = None, batch_size: int = 0):
        ...
        if os.path.exists(self._speaker_dir):
            print("Deleting previous clustering diarizer outputs.")
        ...
        print("Outputs are saved in {} directory".format(os.path.abspath(self._diarizer_params.out_dir)))
    ...
    def restore_from(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                print(f'Model {cls.__name__} was successfully restored from {restore_path}.')
            finally:
                os.chdir(cwd)
